...
st.markdown(f"""
    <div class="status-box">
        ...
        <p style="color:#c5a059; font-weight:bold; margin:0;">● 본인 학번으로 정직하게 신청해 주세요.</p>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("""
    <div class="warning-text">
        ⚠️ [필독] 타인의 학번과 이름을 도용하여 신청하는 행위는 학칙에 따라 엄중히 처벌받을 수 있으며, 
        추후 귀성 및 외출 제한 등 큰 불이익을 초래할 수 있습니다. 반드시 본인 정보로 신청하십시오.
    </div>
    """, unsafe_allow_html=True)
...
with st.container():
    ...
    with col1:
        student_id = st.text_input("학번 (4자리 숫자)", placeholder="예: 1101")
    with col2:
    ...
    # [핵심] 동의 체크박스 가시성 강화
    st.markdown("##### ✍️ 최종 확인")
    confirm_check = st.checkbox("제출 후 수정이 불가능하며, 입력한 정보가 본인의 것임을 확인했습니다.")
    ...
    if st.button("🚀 신청서 제출하기"):
        if not (student_id.isdigit() and len(student_id) == 4):
            ...
            st.warning("위의 동의 체크박스에 체크해 주셔야 제출이 가능합니다.")
        else:
            try:
                # 데이터 읽기 및 저장
                df = conn.read(spreadsheet=SHEET_URL, worksheet="누적_데이터", ttl="0")
                ...
                conn.update(spreadsheet=SHEET_URL, worksheet="누적_데이터", data=updated_df)
                
                st.success(f"🎉 접수 완료! {name} 학생, 신청 데이터가 안전하게 저장되었습니다.")
                st.balloons()
            except Exception as e:
                st.error(f"저장 중 오류 발생! 시트 주소를 확인하세요: {e}")
...
with st.expander("👨‍🏫 학년부 선생님 전용 관리 모드"):
    ...
    if admin_pw == "hanil40":
        try:
            admin_df = conn.read(spreadsheet=SHEET_URL, worksheet="누적_데이터", ttl="0")
